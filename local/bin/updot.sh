...
install_linuxbrew_packages() {
  ...
  brew install zsh git tmux lynx links irssi bmake htop tig \
               ruby python python3 node npm gawk --without-icu4c &&
...
install_python_packages() {
  ...
  pip3 install --user --upgrade \
      autopep8 awscli frosted mkdocs mypy shakyo pylama pylint twine vim-vint
}
...
install_npm_packages() {
  ...
  npm install -g \
      git-recall serverless stylelint https://github.com/so-fancy/diff-so-fancy
}
